...
const buttonCreateNote = document.querySelector(".button-create-notice");
...
function editRow(rowIndex) {
  ...
  const rowData = notes[rowIndex];
  const formFields = form.elements;
  for (const key in rowData) {
    if (formFields[key]) {
      formFields[key].value = rowData[key];
    }
  }
  onOpenModal();
}
...
function renderNotes() {
  ...
  editButtons.forEach((button) => {
    button.addEventListener('click', (event) => {
      ...
      if (row) {
        const rowIndex = parseInt(row.id, 10); // Get the index of the clicked row
        editRow(rowIndex);
      }
    });
  });
